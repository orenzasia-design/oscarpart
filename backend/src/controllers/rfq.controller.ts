...
export const getUserRfqs = async (req: Request, res: Response) => {
  try {
    ...
    const result = await query(`SELECT id, session_number, created_at, status, notes FROM rfq_sessions WHERE user_id = $1 ORDER BY created_at DESC`, [userId]);
...
export const getAllRfqs = async (req: Request, res: Response) => {
  try {
    ...
    if (!['admin', 'superadmin'].includes(userRole)) return res.status(403).json({ success: false, error: 'FORBIDDEN' });
    const result = await query(`SELECT s.id, s.session_number, s.created_at, s.status, s.notes, u.email, u.full_name, u.company_name FROM rfq_sessions s LEFT JOIN users u ON s.user_id = u.id ORDER BY s.created_at DESC`);
    res.json({ success: true, data: result.rows });
  } catch (error) {
...
export const getRfqById = async (req: Request, res: Response) => {
  try {
    ...
    if (sessionResult.rowCount === 0) return res.status(404).json({ success: false, error: 'RFQ_NOT_FOUND' });
...
export const createRFQ = async (req: Request, res: Response) => {
  try {
    ...
    if (!items || !Array.isArray(items) || items.length === 0) return res.status(400).json({ success: false, error: 'ITEMS_REQUIRED' });
    const sessionNumber = `RFQ-${Date.now()}-${Math.floor(Math.random() * 1000)}`;
    const sessionResult = await query(`INSERT INTO rfq_sessions (session_number, user_id, notes, status, created_at, updated_at) VALUES ($1, $2, $3, 'submitted', NOW(), NOW()) RETURNING id, session_number, created_at`, [sessionNumber, userId, notes || '']);
    const session = sessionResult.rows[0];
    ...
    if (userResult.rowCount > 0) {
      ...
      try {
        await sendRFQConfirmationEmail(user.email, user.full_name || 'Customer', session.session_number, partList);
        logger.info(`Email sent for RFQ ${session.session_number}`);
      } catch (emailErr) {
        logger.error(`Email failed: ${emailErr}`);
      }
    }
    res.status(201).json({ success: true, data: { sessionId: session.id, sessionNumber: session.session_number, createdAt: session.created_at } });
  } catch (error) {
